...
export function createExecTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      // ── Ganesh Policy Gate (self-modification deny only) ────────────
      // Evaluates command against Cedar deny rules for self-modification
      // prevention (config, policies, approvals, security services).
      // All other security is handled by OpenClaw's native exec pipeline.
      if (isPolicyGateEnabled()) {
        try {
          ...
          logWarn(`ganesh-policy-gate: evaluation error, failing open: ${String(err)}`);
        }
      }
      // ── End Policy Gate ─────────────────────────────────────────────
